# -*- coding: utf-8 -*-
"""
Created on Mon Jan 19 12:31:05 2026

@author: ChristinaBakatsi
"""
#cd "C:\Users\ChristinaBakatsi\OneDrive - NAMA AE\oasa_ridership_2024"
#streamlit run
import pandas as pd
...
DATA_XLSX_FILES = [
   ...
   #BASE_DIR / "data" / "Per_Day_Per_Stop_Per_Line_November_2025_b_codes.xlsx",
]
...
STOPS_ID_COL = "stop_id"      # in shapefile
EXCEL_STOP_ID_COL = "STOP_ID" # in excel

def norm_stop_id_series(sr: pd.Series) -> pd.Series:
    s = sr.astype(str).str.strip()

    # NEW: remove Excel "text" marker apostrophe if it exists in the data
    s = s.str.lstrip("'")

    # Excel sometimes: 123.0 -> 123
    s = s.str.replace(r"^(\d+)\.0$", r"\1", regex=True)

    # Keep digits only
    s = s.str.replace(r"\D+", "", regex=True)

    # Remove leading zeros (only if you want 000136 -> 136)
    s = s.str.lstrip("0")
...
def load_excel_data(path: Union[str, Path]) -> pd.DataFrame:
    ...
    if path.suffix.lower() == ".csv":
        df = pd.read_csv(
            path,
            dtype={EXCEL_STOP_ID_COL: "string"},
            encoding="utf-8-sig",
            sep=None,
            engine="python",
        )
    else:
        df = pd.read_excel(path, dtype={EXCEL_STOP_ID_COL: "string"})
    ...
    required = {"DAY", "HOUR", "LINE_ROUTE", "STOP_DESCR", EXCEL_STOP_ID_COL, "VALIDATIONS"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Missing required columns: {missing}. Found: {list(df.columns)}")

    # --- Parse DAY ---
    day_raw = df["DAY"].astype(str).str.strip()
    is_iso = day_raw.str.match(r"^\d{4}-\d{2}-\d{2}$")
    df.loc[is_iso, "DAY"] = pd.to_datetime(day_raw[is_iso], format="%Y-%m-%d", errors="coerce")
    df.loc[~is_iso, "DAY"] = pd.to_datetime(day_raw[~is_iso], dayfirst=True, errors="coerce")
    df["DAY"] = pd.to_datetime(df["DAY"], errors="coerce")
    df = df.dropna(subset=["DAY"]).copy()

    # --- Parse HOUR (keep your logic) ---
...
# --- Stop ID and line fields used elsewhere ---
    df["stop_id_norm"] = norm_stop_id_series(df[EXCEL_STOP_ID_COL])
...
def load_excel_many(paths: list[Union[str, Path]]) -> pd.DataFrame:
    ...
    df_all = df_all.drop_duplicates(subset=["date_hour", "line_route", "stop_id_norm"], keep="last")
...
def load_network_geodata(lines_path: Union[str, Path], stops_path: Union[str, Path]):
    ...
    # Validate required stop id column
    if STOPS_ID_COL not in gdf_stops.columns:
        raise ValueError(
            f"Stops shapefile does not contain '{STOPS_ID_COL}'. "
            f"Available columns: {list(gdf_stops.columns)}"
        )

# NEW: stop id key used for matching
    gdf_stops["stop_id_norm"] = norm_stop_id_series(gdf_stops[STOPS_ID_COL])

# Keep stop_descr only for tooltip/display (optional but recommended)
    if STOPS_NAME_COL not in gdf_stops.columns:
       st.warning(
          f"Stops shapefile does not contain '{STOPS_NAME_COL}'. "
          f"Tooltip will show stop_id only."
       )
...
with st.sidebar.expander("Debug: STOP_ID matching", expanded=False):
    excel_ids = set(df["stop_id_norm"].dropna().unique())
    shp_ids = set(gdf_stops["stop_id_norm"].dropna().unique())

    st.write(f"Excel unique STOP_ID: {len(excel_ids):,}")
    st.write(f"SHP unique stop_id: {len(shp_ids):,}")
    st.write(f"Intersection (matchable): {len(excel_ids & shp_ids):,}")
...
with st.sidebar.expander("Debug: SHP stop_id format", expanded=False):
    ...
    st.write("Sample SHP stop_id (raw):")
    st.code(", ".join(shp_raw.dropna().unique()[:30]))

    shp_digits = shp_raw.str.replace(r"\D+", "", regex=True)
    st.write("Sample SHP stop_id (digits-only):")
    st.code(", ".join(shp_digits.dropna().unique()[:30]))

    st.write("Length distribution (digits-only):")
    st.write(shp_digits.dropna().str.len().value_counts().sort_index())
...
stops_belong_set = set(base_for_membership["stop_id_norm"].dropna().unique())
...
gdf_stops_map = gdf_stops_agg[gdf_stops_agg["stop_id_norm"].isin(stops_belong_set)].copy()
